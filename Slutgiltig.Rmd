...
I detta projekt undersöker vi vilka faktorer som bäst kan förklara variationen av självmordsfrekvenen i Sverige baserat på sex förklarande variabler. De givna observationerna är från två olika tidsperioder och vi har valt att titta på modeller för tidsperioderna separat för att inte feltermerna ska bli större. 

Vår metod gick ut på att först observera variablerna parvis mot varandra för att se om det finns några tecken på konlinjäritet, vilket det verkade vara. Efter detta undersökte vi $R^2_{adj}$, $VIF$-värden samt $p$-värden för olika sammansättningar av variabler och utförde variabelselektion genom användning av Backward elimination, Forward selection, Stepwise samt Best adjusted $R^2_{adj}$. Från detta valde vi ut några modeller för period 1 och några modeller för period 2 att resonera kring, där vi sedan valde ut en modell för respektive period som vi ansåg var den bästa förklarande modellen för respektive period. 

Med hjälp av Cooks avstånd undersökte vi även inflytelserika variabler från båda tidsperioderna. Då vi inte kan avgöra vilka av de inflytelserika observationerna som bör exkluderas valde vi att göra hypotetiska modeller där den mest inflytelserika observationen från varje period exkluderades, för att ser hur detta skulle påverka modellen. Sedan gjorde vi på samma sätt som tidigare för dessa dataset där vi valde ut några modeller för period 1 respektive 2 och där vi sedan beslöt oss för en modell för respektive period som vi ansåg bäst beskrev självmordsfrekvensen. 

Sammanfattningsvis har vi fastställt två olika modeller som vi anser är bäst för att förklara självmordsfrekvensen om vi inte tar hänsyn till inflytelserika observationer, där det är en modell för respektive tidsperiod. I den valda modellen för period 1 är de förklarande variablerna alkoholindex, religiositetsindex och arbetslöshetsindex total och för period 2 är de förklarande variabler religiositetsindex, urbaniseringsindex, skilsmässoindex och arbetslöshetsindex total. Vi kommer fram till att alkoholindex verkar kunna vara en bra förklarande variabel, men att kan bytas ut mot skilsmässoindex och urbaniseringsindex, då dessa variabler också kan förklara variationen i självmordsfrekvensen. 
...
Vi börjar med att studera period 1.
...
Vidare har vi också ignorerat förekomsten av inflytelserika observationer i denna del helt och hållet. Vi har valt att göra på det här sättet eftersom vi saknar domänkunskap helt och hållet i detta projekt. Vi anser att det hade varit för spekulativt att studera de här och har istället skapat en egen sektion av rapporten som är helt dedikerat till inflytelserika observationer och vad som händer om man tar bort dem.

```{r}
(lm(data=data_1, formula = sjmo ~ alko + arblösTot + religio ))
```

Sett till den första modellen så ser vi att alkohol verkar ha ett positivt
samband till självmordsfrekvensen, vilket är föga överraskande. Mer intressant
blir kanske den andra modellen.

```{r}
(lm(data=data_1, formula = sjmo ~ skilsmässo + arblösTot + religio + urban ))
```

Om vi tittar på estimationerna här ser vi att koefficienten för urbaniseringsindex är nära 0, alltså att den har relativt liten påverkan på självmord. Den största skillnaden för de två modellerna är att den ena har med skillsmässoindex och den andra har med alkoholindex. Både modellen med alkoholindex och modellen med skilsmässoindex är bra förklaringsmodeller för självmordsfrekvens. Det är också värt att notera att dessa är två variabler som är ganska högt korrelerade (alltså alkohol och skilsmässor).

För att ytterligare stärka dessa modeller, undersöker vi hur deras residualer
ser ut.
...
Vi går nu vidare till period 2.
...
I Tabell 2 kan vi observera resultaten för period 2. Vi kan se att modellen med högst  $R^2_{adj}$ också har väldigt höga VIF-värden. Därför exkluderar vi denna från de potentiellt bästa modellerna. Däremot har modellen med näst högst $R^2_{adj}$ relativt bra VIF och relativt låga p-värden. Denna modell har arbetslöshetsindex, urbaniseringindex, religionsindex och skilsmässoindex som förklarande variabler, vilket även var en av de bästa modellerna för period 1. Vi har alltså inte med alkoholindex i den bästa modellen
för period 2.

Vi kan nu se till koefficienterna för denna bästa modell i relation till kandidaten
som inkluderar alkohol (och urbanisering).

Först modellen utan alkohol:

```{r}
lm(data=data_2, formula = sjmo ~ arblösTot + urban + skilsmässo + religio)
```
Sedan utan alkohol:

```{r}
lm(data=data_2, formula = sjmo ~ alko + urban)
```
Jämför vi nu de två så ser vi att, speciellt i den andra modellen så bidrar 
urbaniseringsindex-variabeln med väldigt lite förklaring - lutningen är nära
noll. Dessutom ser vi att skilsmässo-index och alkohol är ungefär lika
stor effekt (nästan samma koefficient).
