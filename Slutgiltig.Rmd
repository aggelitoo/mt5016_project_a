...
Till vår hjälp har vi två datamängd baserade på registerdata från svenska län under 1960-talet, med tidsperioderna 1963-65 (period 1) och 1966-1968 (period 2). Dessa datamängd innehåller information från 25 län och omfattar sex variabler som tros kunna påverka självmordsfrekvensen i länen. De undersökta variablerna inkluderar:
...
```{r, echo=FALSE}
# datamängd 1 adjusted r sq plot
all_1 %>%
...
```{r, echo=FALSE}
# datamängd 2 adjusted r sq plot
all_2 %>%
ggplot(aes(x=n, y=adjr, label = predictors)) +
  geom_point() +
...
```{r, echo=FALSE}
# datamängd 1 with jönköping removed adjusted r sq plot
all_1_red %>%
ggplot(aes(x=n, y=adjr, label = predictors)) +
  geom_point() +
...
```{r, echo=FALSE, warning=FALSE}
# datamängd 2 with norrbotten removed adjusted r sq plot
all_2_red %>%
ggplot(aes(x=n, y=adjr, label = predictors)) +
  geom_point() +
